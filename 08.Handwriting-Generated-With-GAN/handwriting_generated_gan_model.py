...
class HandwritingGANModel(object):
    def __init__(self, batch_size=64, learning_rate=1e-3):
        # 1. Define input.
        self.input_image = tf.placeholder(tf.float32, [batch_size, 28 * 28],
                                          name="input_image")
        ...
        generator_w3 = tf.Variable(
            tf.truncated_normal([300, 28 * 28], stddev=0.1),
            name="g_w3", dtype=tf.float32)
        generator_b3 = tf.Variable(tf.zeros([28 * 28]), name="g_b3",
                                   dtype=tf.float32)
        ...
        discriminator_w1 = tf.Variable(
            tf.truncated_normal([28 * 28, 300], stddev=0.1),
